...
//		$this->uriparameter = 'search=positions&closed=0&show=json';
	...
	public function map_to_schema($data)
	{
		...
		if (isset($data['Position'])) {
			foreach ($data['Position'] as $num => $job) {
				if (is_array($job) && isset($job['title'])) {
	...
	private function sanitize_univis_typen($value, $type = 'type1')
	{
		$validselectors = [
			...
			"type3" => [
				'vertr' => __('Replacement', 'rrze-jobs'),
				// 'Vertretung',
				'schwanger' => __('Maternity leave replacement', 'rrze-jobs'),
				// 'Mutterschutzvertretung', 
				'eltern' => __('Maternity/parental leave replacement', 'rrze-jobs'),
				// 'Mutterschutz- / Elternzeitvertretung',
				'forsch' => __('Limited research project', 'rrze-jobs'),
				// 'befristetes Forschungsvorhaben',
				'krankh' => __('Sickness replacement', 'rrze-jobs'),
				// 'befristetes Krankheitsvertretung',
				'zeitb' => __('Temporary officials', 'rrze-jobs'),
	...
	private function sanitize_univis_textfeld($txt)
	{
		$subs = array(
			...
			// list
			'/[\n\r\s]+<\/li>/mi' => '</li>',
			// remove breaks at end of listelements
			'/(<\/ul>\n(.*)<ul>*)+/' => '',
			// list
			'/\*{2}/m' => '/\*/',
			// **
			...
			'/\^(.*)\^/m' => '<sup>$1</sup>',
			// pi^2^
			'/\[([^\]]*)\]\s{0,1}((http|https|ftp|ftps):\/\/\S*)/mi' => '<a href="$2">$1</a>',
			// [link text] http...
			'/\[([^\]]*)\]\s{0,1}(mailto:)([^")\s<>]+)/mi' => '<a href="mailto:$3">$1</a>',
			// find [link text] mailto:email@address.tld but not <a href="mailto:email@address.tld">mailto:email@address.tld</a>
